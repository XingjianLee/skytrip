...
const DashboardMainNew = () => {
  ...
  const hotels = [
    ...
    },
    {
      ...
      price: "¥980",
      image:
        "https://images.unsplash.com/photo-1564501049415-37f5f6063419?q=80&w=2670&auto=format&fit=crop",
    },
  ];
            ...
            <div className="relative h-full flex flex-col p-6 text-white">
              ...
              <div className="flex flex-wrap gap-2 mt-6">
                {activities.map((activity) => (
                  <button
                    key={activity.id}
                    onClick={() =>
                      setActiveActivity(activity.id === activeActivity ? null : activity.id)
                    }
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs backdrop-blur-md transition-all border ${activeActivity === activity.id
              ...
              <div className="mt-auto pt-8 space-y-4">
                ...
                {trips.map((trip) => (
                  <div
                    key={trip.id}
                    className="bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-3 hover:bg-white/20 transition-colors cursor-pointer flex gap-3 group/card"
              ...
              <div className="flex items-center justify-between mb-6">
                <div className="flex-1 flex items-center justify-between bg-slate-50 dark:bg-slate-900 rounded-2xl p-4 border border-slate-100 dark:border-slate-800">
                  ...
                  <Button
                    ...
                    size="icon"
                    onClick={handleSwap}
                    className="rounded-full hover:bg-white hover:shadow-sm text-slate-400"
                  >
                ...
                <Button
                  className="ml-4 h-auto py-4 rounded-2xl bg-slate-900 text-white hover:bg-slate-800 shadow-lg shadow-slate-900/20"
              ...
              <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
                {dates.map((date, index) => (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center min-w-[60px] py-3 rounded-2xl border transition-all ${selectedDateIndex === index
          ...
          <div className="space-y-4">
            ...
            {recommendedRoutes.map((route) => (
              <div
                ...
                onClick={() => navigate("/book-flight")}
              >
                <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${route.color}`} />
                <div className="flex justify-between items-center pl-3">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <Badge
                        variant="secondary"
                        className="rounded-md text-[10px] bg-slate-100 text-slate-600 font-normal"
                      >
                        {route.popularity}
                      </Badge>
                    </div>
                    ...
                    <div className="mt-3 flex gap-2">
                      {route.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-[10px] px-2 py-0.5 bg-slate-50 text-slate-500 rounded border border-slate-100"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className="text-right">
                    <div className="text-xs text-slate-400 line-through mb-1">
                      ¥{Math.round(route.priceFrom * 1.2)}
                    </div>
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      className="h-7 mt-1 text-xs group-hover:translate-x-1 transition-transform text-amber-600 hover:text-amber-700 hover:bg-amber-50"
                    >
        ...
        <div className="lg:col-span-4 space-y-6 flex flex-col">
          <div className="relative rounded-3xl overflow-hidden bg-slate-900 text-white shadow-2xl shadow-slate-300 dark:shadow-none group h-[400px]">
            ...
            <div className="absolute top-4 left-4 right-4 flex justify-between items-start z-10">
              ...
              <Button
                size="icon"
                className="rounded-full bg-white/20 backdrop-blur-md hover:bg-white text-white hover:text-red-500 border-0"
              >
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
              ...
              <Button
                className="w-full bg-white text-slate-900 hover:bg-slate-100 rounded-xl font-bold"
                onClick={() => navigate("/book-ticket")}
              >
                查看攻略
              </Button>
          ...
          <div className="bg-white dark:bg-card rounded-3xl p-5 shadow-sm border border-slate-100 dark:border-slate-800 flex-1">
            ...
            <div className="space-y-4">
              {hotels.map((hotel) => (
                <div
                  key={hotel.id}
                  className="group flex gap-3 p-2 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-2xl transition-all cursor-pointer"
                >
                  ...
                  <div className="flex-1 flex flex-col justify-between py-0.5">
                    <div>
                      <h5 className="font-bold text-slate-800 dark:text-white text-sm line-clamp-1">
                        {hotel.name}
                      </h5>
            ...
            <div className="flex justify-end">
              <Button variant="link" className="text-xs" onClick={() => navigate("/book-hotel")}>
                更多住宿
              </Button>
            </div>
          </div>
...
export default DashboardMainNew;
